...
- [x] Индикатор статуса модерации на карточках (pending / rejected + причина)
- [x] Страница: «Мои публикации» — статус с объяснением что делать при отклонении
- [x] Кнопка «Исправить и отправить повторно» (переводит в draft → edit → submit)
- [x] Уведомление в UI (toast) о смене статуса модерации
...
- [ ] Внешние вакансии: парсер (спецификации пока нет — см. `docs/roadmap.md`)
