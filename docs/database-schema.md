...
**Важно:** поле статуса модерации в Company/Vacancy/Resume называется `moderationStatus` (имя `status` зарезервировано Strapi 5 для draft&publish).
...
| avatar                       | string (URL), nullable      | URL: Telegram CDN или собственный S3 (allowlist) |
...
| boostCredits                 | int                         | Default: 0, из пакетов                           |
| isVip                        | boolean                     | Default: false; true когда plan=vip              |
| telegramNotificationsEnabled | boolean                     | Default: true; тумблер в настройках              |
| confirmed                    | boolean (users-permissions) | Email подтверждён                                |
...
| moderationStatus | enum(draft, moderation, published, rejected)                                                                | Default: draft (legacy, флоу: создание → moderation)            |
...
**Индексы:** `slug`, `moderationStatus`, `owner`
...
| moderationStatus  | enum(draft, moderation, published, rejected, expired, archived)                                             | Default: draft (legacy, флоу: создание → moderation) |
...
**Индексы:** `moderationStatus`, `industry`, `specialization`, `country`, `workFormat`, `employmentType`, `seniority`, `sourceType`, `expiresAt`, `topPlacement`, `urgent`, `company`
...
| moderationStatus | enum(draft, moderation, published, rejected, archived)                                                      | Default: draft (legacy, флоу: создание → moderation) |
| rejectionReason  | enum(spam, fake, inappropriate, incomplete, wrong_category, salary_mismatch, contact_info, other), nullable | Причина отклонения                                   |
| rejectionComment | text, nullable                                                                                              | Комментарий модератора (виден автору)                |
| createdAt        | datetime, auto                                                                                              |                                                      |
...
**Индексы:** `user`, `moderationStatus`, `country`, `workFormat`, `employmentType`
...
- Кандидат: `resume_viewed`, `application_approved`, `application_rejected`, `interview_invitation`, `test_task`, `offer_received`, `subscription_expiring`, `subscription_expired`, `moderation_approved`, `moderation_rejected`, `saved_search_match` (legacy — модуль сохранённых поисков удалён, значение осталось в enum)
...
Журнал платежей Telegram Stars. Создаётся при обработке `successful_payment` в webhook.

| Поле             | Тип                                          | Описание                                 |
| ---------------- | -------------------------------------------- | ---------------------------------------- |
| id               | int, PK, auto                                |                                          |
| user             | relation → User                              | many-to-one                              |
| telegramChargeId | string, unique                               | ID транзакции Telegram (идемпотентность) |
| payloadType      | enum(subscription, vacancy_pack, apply_pack) |                                          |
| planCode         | string, nullable                             | Для subscription: pro/max/vip            |
| packageId        | int, nullable                                | Для пакетов: id пакета                   |
