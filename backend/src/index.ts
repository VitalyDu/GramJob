...
import { setupContentManagerViews } from './scripts/setup-content-manager'
...
async function setupVacancySearch(strapi: Core.Strapi) {
  try {
    // Idempotent column renames: status → moderation_status
    await strapi.db.connection.raw(`
      DO $$ BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='vacancies' AND column_name='status') THEN
          ALTER TABLE vacancies RENAME COLUMN status TO moderation_status;
        END IF;
      END $$;
    `)
    await strapi.db.connection.raw(`
      DO $$ BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='resumes' AND column_name='status') THEN
          ALTER TABLE resumes RENAME COLUMN status TO moderation_status;
        END IF;
      END $$;
    `)
    await strapi.db.connection.raw(`
      DO $$ BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='companies' AND column_name='status') THEN
          ALTER TABLE companies RENAME COLUMN status TO moderation_status;
        END IF;
      END $$;
    `)
    await strapi.db.connection.raw(`
      DROP INDEX IF EXISTS vacancies_status_expires_idx;
    `)
    ...
    await strapi.db.connection.raw(`
      ...
      CREATE INDEX IF NOT EXISTS vacancies_moderation_status_expires_idx ON vacancies (moderation_status, expires_at);
...
export default {
  ...
  async bootstrap({ strapi }: { strapi: Core.Strapi }) {
    ...
    await setupContentManagerViews(strapi)
